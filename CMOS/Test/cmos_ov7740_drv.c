...
static unsigned long *GPJCON;		//摄像头控制寄存器
...
static unsigned long *CIPRCTRL;			//preview DMA Control Register
...
static void cmos_ov7740_calculate_scaler_info(void)
{
	...
	sx = SRC_Width;		//目标图片的宽度
	sy = SRC_Height;
	tx = TargetHsize_Pr;	//目标图片的高度
	ty = TargetVsize_Pr;

	printk("%s: SRC_in(%d, %d), Target_out(%d, %d)\n", __func__, sx, sy, tx, ty);

	//水平方向的比、变比，垂直方向的比，V_Shift变比
...
static int cmos_ov7740_vidioc_streamon(struct file *file , void *priv, 
								enum v4l2_buf_type i)
{
	...
	*CIWDOFST |= (1<<31) | (WinHorOfst<<16) | (WinVerOfst<<0);
	...
	CIGCTRL		全局控制寄存器
		...
		bit[30]		-- 用于复位外部摄像头模块
		bit[29]		-- 保留位，必须设置为1
		bit[28:27]	-- 用于选择信号源(00 = 输入源来自外部摄像头，01-11 内置测试数据)
		bit[26]		-- 设置像素时钟的极性（猜0）
		bit[25]		-- 设置VSYNC的极性（0）		帧同步的极性
		bit[24]		-- 设置HREF的极性（0）		行同步的极性
	*/
...
static const struct v4l2_ioctl_ops cmos_ov7740_ioctl_ops = {
	...
	/*  说明，因为我们是通过读的方式来获得摄像头数据的，因此查询/放入队列/取出队列这些操作将不再需要 */
#if 0
										...
										const struct i2c_device_id *id)
{
	...
	//预览通道，用来分配缓冲区的数据，在cmos_ov7740_vidioc_reqbufs申请缓冲区
	...
	/* 2.3.7 注册中断 irq中断，对于摄像头来说有两个中断源：IRQ_CAM_C IRQ_CAM_P	*/
