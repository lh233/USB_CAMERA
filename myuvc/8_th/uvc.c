...
/* A8 查询缓存状态，比如地址信息（APP可以使用mmap函数进行映射） VIDIOC_QUERYBUF
*
*参考: uvc_query_buffer
*/
...
static int myuvc_try_streaming_params(struct myuvc_streaming_control *ctrl)
{
    ...
    int ret;
	__u8 type = USB_TYPE_CLASS | USB_RECIP_INTERFACE;
	//控制传输是每种USB设备中都会存在的USB传输方式，控制传输用于主机同设备的控制端点进行通信，通过读取设备的配置信息来完成对设备的枚举和配置
	//而管道类似于一种邮政编码，
	unsigned int pipe;
    ...
    *(__le16 *)&data[16] = cpu_to_le16(ctrl->wDelay);
    ...
    pipe = (SET_CUR & 0x80) ? usb_rcvctrlpipe(myuvc_udev, 0)
                  : usb_sndctrlpipe(myuvc_udev, 0);
    type |= (SET_CUR & 0x80) ? USB_DIR_IN : USB_DIR_OUT;	//设置USB的传输方向
