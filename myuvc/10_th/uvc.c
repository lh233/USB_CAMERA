...
static struct frame_desc frames[] = {{640, 480},  {320, 240},  {160, 120}};		//支持哪几种分辨率，韦东山的摄像头支持3种分辨率
static int frame_idx = 1;
static int bBitsPerPixel = 0;		///* lsusb -v -d 0x1b3b:  "bBitsPerPixel" */一个数据占据多少字节，表示几位像素，因为这是MJEP格式的，是压缩的格式
static int uvc_version = 0x0100; /* lsusb -v -d 0x1b3b: bcdUVC , uvc 版本号 */
...
/* lsusb -v -d 0x1b3b: PROCESSING_UNIT */
...
static int dwMaxVideoFrameSize = 77312;			//表示一帧视频最大的长度，是从print_parms中得到的，具体在probe函数中

static int myuvc_bEndpointAddress = 0x82;	//端点传输地址，用于USB传输使用，在韦东山的视频中是0x82，videoStreaming Interface 是0x82


static int last_fid = -1;	//初始值为-1
...
*/
	...
	*/
	...
	f->pixelformat = V4L2_PIX_FMT_MJPEG;		//设置为MJPEG格式
...
static void myuvc_video_complete(struct urb *urb)
{
	...
	for (i = 0; i < urb->number_of_packets; ++i) 
	{
		...
		/* fid != last_fid 表示开始新一帧了，表示已经接收到了新数据 */
...
static int myuvc_vidioc_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	*/
	 ...
	 * 可以看到bAlternateSetting为第5个setting
     * bAlternateSetting       5
     */
...
//	struct usb_interface_assoc_descriptor *assoc_desc;
    ...
    if (cnt == 2)
    {
		...
		//insmod驱动之后，会输出相应的信息：
		//[ 4020.000348] usbcore: deregistering interface driver uvcvideo
		/*
			[ 4027.141723] myuvc_probe : cnt = 0
			[ 4027.141732] myuvc_probe : cnt = 1
			[ 4027.416184] myuvc_try_streaming_params ret = 0
			[ 4027.417914] myuvc_get_streaming_params ret = 0
			[ 4027.419199] myuvc_set_streaming_params ret = 0
			[ 4027.419202] video params:
			[ 4027.419204] bmHint                   = 1
			[ 4027.419206] bFormatIndex             = 1
			[ 4027.419207] bFrameIndex              = 2
			[ 4027.419209] dwFrameInterval          = 333333
			[ 4027.419210] wKeyFrameRate            = 0
			[ 4027.419212] wPFrameRate              = 0
			[ 4027.419213] wCompQuality             = 0
			[ 4027.419215] wCompWindowSize          = 0
			[ 4027.419216] wDelay                   = 0
			[ 4027.419218] dwMaxVideoFrameSize      = 77312
			[ 4027.419220] dwMaxPayloadTransferSize = 800
			[ 4027.419221] dwClockFrequency         = 0
			[ 4027.419223] bmFramingInfo            = 0
			[ 4027.419242] bPreferedVersion         = 0
			[ 4027.419244] bMinVersion              = 0
			[ 4027.419246] bMinVersion              = 0
		*/
    }
