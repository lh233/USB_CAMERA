...
static struct myuvc_streaming_control myuvc_params;		//设置UVC数据发送包


static struct video_device *myuvc_vdev;		//视频设备
static struct usb_device *myuvc_udev;		//USB设备
static int myuvc_control_intf;				//video_control接口


static int wMaxPacketSize = 800;			//一次性传输的数据为800，在streamon函数中

static int myuvc_bEndpointAddress = 0x82;	//端点传输地址，用于USB传输使用，在韦东山的视频中是0x82，videoStreaming Interface 是0x82
...
static int frame_idx = 1;			//将分辨率设置为第一种640*480分辨率格式的


static int bBitsPerPixel = 0;		/* lsusb -v -d 0x1b3b:  "bBitsPerPixel" 一个数据占据多少字节，表示几位像素，因为这是MJEP格式的，是压缩的格式 */


static int myuvc_streaming_bAlternateSetting = 5;		//第5个setting
...
static int ProcessingUnitID = 3;			//详细请查看UVC 1.5 Class specification.pdf中的Process Unit Descriptor中的bUnitID就可以了，用来设置相应的属性的
...
static int myuvc_vidioc_enum_fmt_vid_cap(struct file *file, void *priv, struct v4l2_fmtdesc *f)
{
	...
	*/
	...
	f->pixelformat = V4L2_PIX_FMT_MJPEG;		//设置为MJEP格式
...
{
	memset(f,  &myuvc_format, sizeof(myuvc_format));
	return 0;
}

/* A5 测试驱动程序是否支持某种格式，强制设置某种格式 */
static int myuvc_vidioc_try_fmt_vid_cap(struct file *file, void *priv, struct v4l2_format *f)
{
	...
	f->fmt.pix.height = frames[frame_idx].height;
	 ...
	 //一个像素占几个字节，直接设置为0，因为是压缩数据
	f->fmt.pix.bytesperline = (f->fmt.pix.width * bBitsPerPixel) >> 3;
	
	
	f->fmt.pix.field 	= V4L2_FIELD_NONE;	/* this device has no fields ... */
	f->fmt.pix.colorspace = V4L2_COLORSPACE_SRGB;			/* V4L2_COLORSPACE_SRGB.多数开发者所熟悉的［red,green,blue]数组包含在这个色域之中。*/
...
static int myuvc_vidioc_reqbufs(struct file* file , void * priv, struct v4l2_requestbuffers *p)
{
	...
	for(i=0; i<nbuffers; i++)
	{
		...
		myuvc_queue.buffer[i].buf.m.offset = i * bufsize;		//偏移地址
...
static int myuvc_get_straming_params(struct myuvc_streaming_control *ctrl)
{
	__u8 *data;
	__u16 size;
	int ret;
...
static int myuvc_alloc_init_urbs(void)
{
	...
	int ret=0;	
	psize = wMaxPacketSize; 		/* 实时端点传输的最大字节数 */
	size = myuvc_params.dwMaxVideoFrameSize;		/* 实时传输一帧数据最大长度 */
	...
	for(i=0; i<MYUVC_URBS; i++)
	{
		/* 1.分配usb_buffers，也就是用来申请usb_device的内存 */
		myuvc_queue.urb_buffer[i] = usb_buffer_alloc(myuvc_udev, size, GFP_KERNEL|__GFP_NOWARN, &myuvc_queue.urb_dma[i]);
	...
	/* 3.设置urb，等时传输才需要这样做，详情查看Linux设备开发详解 */
	for(i=0; i<MYUVC_URBS; i++) {
		urb = myuvc_queue.urb[i];
		
		urb->dev = myuvc_udev;
		urb->context = NULL;
		urb->pipe = usb_rcvctrlpipe(myuvc_udev, myuvc_bEndpointAddress);		//设置端点
		urb->interval = 1;
...
static int myuvc_vidioc_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
    ...
    /* b. 取出参数 */
    ret = myuvc_get_straming_params(&myuvc_params);
...
/* A13 APP通过poll命令确认缓冲区有无数据后，把缓冲区队列中取出来 */
...
static int myuvc_vidioc_g_ctrl(struct file *file, void *fh, struct v4l2_control *ctrl)
{
	__u8 type = USB_TYPE_CLASS | USB_RECIP_INTERFACE;
	unsigned int pipe;
	int ret;
...
/* A12 APP使用poll/select函数来确定函数是否就绪（有数据） */
...
static int myuvc_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	if(cnt == 2)
	{
		...
		/* 3.注册 */
		//VFL_TYPE_GRABBER 表明是一个图像采集设备–包括摄像头、调谐器
...
{
	static int cnt=2;
	printk("myuvc_disconnect is : cnt is %d " ,cnt++);
